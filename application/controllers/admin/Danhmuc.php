...
class Danhmuc extends MY_Controller
{
    ...
    {
        $input = array();
        $list = $this->Danhmuc_model->get_list($input);
        $this->data['list'] = $list;
    ...
    function ten_danhmuc()
    {
        $TENDM = $this->input->post('TENDM');
        $where = array('TENDM' => $TENDM);

        if ($this->Danhmuc_model->check_exists($where)) {
    ...
    {
        if ($this->input->post()) {
            $this->form_validation->set_rules('TENDM', 'Tên danh mục', 'required|callback_ten_danhmuc');
        ...
        $this->success_form['temp'] = 'admin/danhmuc/add';
        ...
        if ($info == FALSE) {
            ...
            if ($this->input->post()) {
                ...
                if($this->form_validation->run() == TRUE) {
                    ...
                    if ($this->Danhmuc_model->update($MADM, $data)) {
    ...
    function delete()
    {
        $message = '';
        $this->success_form['message'] = $message;
        $TENDM = $this->uri->rsegment('3');
        $info = $this->Danhmuc_model->get_info($TENDM);
        if ($info == FALSE) {
            $message = '<div class="alert alert-danger" role="alert">Dữ liệu không tồn tại</div>';
            $this->success_form['message'] = $message;
        } else {
            //thực hiện xóa
            $this->Danhmuc_model->delete($TENDM);
